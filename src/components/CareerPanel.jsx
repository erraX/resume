          ...
          <li className="mb-4">
            <Project
              ...
              renderDesc={() => (
                <>
                  <div>
                    合约广告库存询量、下单后台管理系统。项目陈旧(10年)，原生 js
                    开发，前后端未分离、无打包工具、手动部署上线。业务逻辑复杂，其中表单字段多(几十个)、联动多(多对多、循环、链式、异步联动)、各种特殊规则
                  </div>
                </>
              )}
              items={[
                '模块化改造：使用 babel 来编译非标准的模块至 CommonJS，能够使用 npm 管理依赖包、使用 webpack 打包',
                '自动化部署：由手动调用接口, php 编译 js ，上传代码至 git 仓库部署 => 升级为 通过 ci 线上自动化编译',
        ...
        <ul className="list-circle pl-4">
          <li>
            <Project
              ...
              items={[
                '项目架构较老(jQuery)，对页面进行了 MVC 分离',
                '标准化表单校验，内置各种常用校验规则，校验模块能适配不同 UI 组件',
                '在原有基础上开发数据绑定层，兼容老框架，能够对 View - Model 进行双向绑定',
              ]}
            />
          </li>
          <li>
